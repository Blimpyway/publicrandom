...
# - is set to a number > 0 it repeats ROUNDS MEMHASHLOOPS times , 
...
# a 256 MB array, big enough for "desktop" hashes
MEMHASHSIZE=67108864
...
# Creates a 256MB random.bin file, should well exceed most CPU caches
cd `dirname $0`
[ -f "./random.bin" ] || {
    echo -n "Creating random.bin from /dev/urandom..."
    dd if=/dev/urandom of=random.bin bs=4096 count=65536
    echo "done!"
}
